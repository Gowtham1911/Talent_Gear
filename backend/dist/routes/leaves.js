...
router.post("/", auth_1.authenticate, async (req, res) => {
    ...
    const [rows] = await mysql_1.default.query("SELECT id FROM employees WHERE user_id = ? LIMIT 1", [req.user.id]);
    if (!rows.length) {
        res.status(404).json({ message: "Employee profile not found" });
        return;
    }
    const employeeId = rows[0].id;
    const [result] = await mysql_1.default.query("INSERT INTO leaves (employee_id, type, from_date, to_date, days, reason) VALUES (?, ?, ?, ?, ?, ?)", [employeeId, type || "annual", from_date, to_date, days, reason || null]);
...
router.get("/my", auth_1.authenticate, async (req, res) => {
    const [rows] = await mysql_1.default.query(`SELECT l.* FROM leaves l
...
router.get("/upcoming", auth_1.authenticate, auth_1.requireAdmin, async (_req, res) => {
    const [rows] = await mysql_1.default.query(`SELECT l.*, e.first_name, e.last_name, e.department, e.position
     FROM leaves l
     JOIN employees e ON l.employee_id = e.id
...
router.patch("/:id", auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    ...
    await mysql_1.default.query("UPDATE leaves SET status = ?, admin_note = ? WHERE id = ?", [status, admin_note || null, id]);
    res.json({ message: `Leave ${status}` });
});
// Admin: delete a leave request
router.delete("/:id", auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    await mysql_1.default.query("DELETE FROM leaves WHERE id = ?", [Number(req.params.id)]);
